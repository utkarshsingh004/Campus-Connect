...
import { FiMenu, FiX, FiMoon, FiSun, FiLogIn, FiLogOut } from "react-icons/fi";

function Navbar() {
  const [isOpen, setIsOpen] = useState(false);
  const { darkMode, toggleDarkMode } = useTheme();
  const { isLoggedIn, logout, hostIsLoggedIn, hostLogout } = useAuth();
  ...
  // Automatically logout host if user logs in
  useEffect(() => {
    if (isLoggedIn && hostIsLoggedIn) {
      hostLogout();
    }
  }, [isLoggedIn, hostIsLoggedIn, hostLogout]);

  // Automatically logout user if host logs in
  useEffect(() => {
    if (isLoggedIn && hostIsLoggedIn) {
      logout();
    }
  }, [isLoggedIn, hostIsLoggedIn, logout]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex md:items-center md:space-x-4">
            ...
            {/* Show User's Dashboard only if user is logged in */}
            {isLoggedIn && !hostIsLoggedIn && (
              <NavLink
                to="/dashboard"
                className={({ isActive }) =>
                  `px-3 py-2 rounded-md text-sm font-medium ${
            ...
            {/* Show Host's Dashboard only if host is logged in */}
            {hostIsLoggedIn && !isLoggedIn && (
              <NavLink
                to="/host/dashboard"
                className={({ isActive }) =>
                  `px-3 py-2 rounded-md text-sm font-medium ${
